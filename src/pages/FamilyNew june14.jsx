  ...
  try {
    ...
    if (typeof dateString === 'string' && dateString.includes('/')) {
      ...
      if (parts.length === 3) {
        ...
        const year = parseInt(parts[2]);
        transactionDate1 = new Date(year, month - 1, day); // month is 0-indexed
      }
    } else {
      // Fallback to normal date parsing
      transactionDate1 = new Date(dateString);
    }
    
    // Get the financial year based on transaction date
    const transactionMonth = transactionDate1.getMonth() + 1; // 1-12
    const transactionYear = transactionDate1.getFullYear();
    
    // Financial year logic: Apr-Mar
    // If month is Jan-Mar (1-3), financial year is previous year
    // If month is Apr-Dec (4-12), financial year is current year
    return transactionMonth <= 3 ? transactionYear - 1 : transactionYear;
