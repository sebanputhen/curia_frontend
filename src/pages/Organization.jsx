...
const StyledCard = styled(Card)(() => ({
  ...
  "&:hover": { transform: "translateY(-4px)", boxShadow: "0 10px 15px -3px rgba(0,0,0,0.1), 0 4px 6px -2px rgba(0,0,0,0.05)" },
}));
...
const IconBox = styled(Box)(({ color }) => ({
  ...
  background: `linear-gradient(135deg, ${color}20, ${color}40)`, color,
}));

const ChartCard = styled(Card)(({ theme }) => ({
  backgroundColor: "#fff", borderRadius: 16, padding: theme.spacing(3),
  boxShadow: "0 4px 6px -1px rgba(0,0,0,0.1), 0 2px 4px -1px rgba(0,0,0,0.06)",
}));

const GradientBtn = styled(Button)(() => ({
  background: "linear-gradient(135deg, #1a237e, #0d47a1)", color: "#fff", borderRadius: 12,
  padding: "10px 24px", fontWeight: 600, textTransform: "none",
  "&:hover": { background: "linear-gradient(135deg, #0d47a1, #1a237e)" },
}));

const Field = styled(TextField)(() => ({
  "& .MuiOutlinedInput-root": {
    borderRadius: 12, "& fieldset": { borderColor: "#E2E8F0" },
    "&:hover fieldset": { borderColor: "#0d47a1" },
    "&.Mui-focused fieldset": { borderColor: "#1a237e" },
  },
}));

const GradientText = ({ children, variant = "h5", sx = {} }) => (
  <Typography variant={variant} sx={{
    fontWeight: 700, background: "linear-gradient(45deg, #1a237e, #0d47a1)",
    backgroundClip: "text", WebkitBackgroundClip: "text", color: "transparent", ...sx,
  }}>{children}</Typography>
);
...
const Organization = () => {
  ...
  const load = async () => {
    setLoading(true);
    try { setRows((await axiosInstance.get("/organization/")).data); }
    catch { setMsg({ type: "error", text: "Failed to fetch" }); }
  ...
  const del = async (id) => {
    ...
    try { await axiosInstance.delete(`/organization/${id}`); load(); setMsg({ type: "success", text: "Deleted" }); }
  ...
  const submit = async (e) => {
    ...
    setLoading(true);
    try {
      if (editing) await axiosInstance.put(`/organization/${selected._id}`, { name: name.trim() });
      else await axiosInstance.post("/organization", { name: name.trim() });
      load(); setOpen(false); reset();
      ...
      Cell: ({ row }) => (
        <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
          <Box sx={{ width: 36, height: 36, borderRadius: 2, flexShrink: 0, background: "linear-gradient(135deg, #E8EAF620, #E8EAF660)", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <BusinessIcon fontSize="small" sx={{ color: "#1a237e" }} />
          </Box>
          <Typography variant="body1" sx={{ fontWeight: 600, color: "#0F172A" }}>{row.original.name}</Typography>
        </Box>
      ),
    },
    {
      accessorKey: "createdAt", header: "Created", size: 150,
      Cell: ({ row }) => <Typography variant="body2" sx={{ color: "#64748B" }}>{row.original.createdAt ? new Date(row.original.createdAt).toLocaleDateString() : "—"}</Typography>,
    },
    {
      id: "actions", header: "Actions", size: 110,
      Cell: ({ row }) => (
        <Box sx={{ display: "flex", gap: 1 }}>
          <Tooltip title="Edit"><IconButton size="small" onClick={() => edit(row.original)} sx={{ bgcolor: "#E8EAF6", "&:hover": { bgcolor: "#C5CAE9" } }}><EditIcon fontSize="small" sx={{ color: "#1a237e" }} /></IconButton></Tooltip>
          <Tooltip title="Delete"><IconButton size="small" onClick={() => del(row.original._id)} sx={{ bgcolor: "#FEE2E2", "&:hover": { bgcolor: "#FECACA" } }}><DeleteIcon fontSize="small" sx={{ color: "#DC2626" }} /></IconButton></Tooltip>
        </Box>
      ),
    },
  ], []);
  ...
  return (
    <Page>
      ...
      <Dialog open={open} onClose={() => { setOpen(false); reset(); }} maxWidth="sm" fullWidth TransitionComponent={Fade}
